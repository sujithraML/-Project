...
<header>
 	 <nav class="nav">
   	   <div class="container">
	        ...
	        <a class="brand" href="#">Wanderlust</a>
	   	  ...
	   	  <div class="menu">
	    	<ul class="list">
				<li><form action="UserLogout" method="post"><button type="submit" >Logout</button></form>
		   </ul>
	   	 </div>   
 		</div>
 	</nav>
  	...
  	<div class="form-group">
  	 <form action="SelectFlightDeals" method="post">
  	 </form>
 	...
 	while(iterator.hasNext()){
		...
		++i;	
			%>
			<tr>
				<td><%=((Flight)flight).getFlightId() %> </td>
				<td><%=((Flight)flight).getAirLineName() %> </td>
				<td><%=((Flight)flight).getFromLocation() %> </td>
				<td><%=((Flight)flight).getToLocation() %> </td>
				<td><%=((Flight)flight).getDepartureTime() %> </td>
				<td><%=((Flight)flight).getArrivalTime() %> </td>
				<td><%=((Flight)flight).getDuration() %> </td>
				<td><%=((Flight)flight).getBaggageAllowance() %> </td>
				<td><%=((Flight)flight).getFlightClass() %> </td>
				<td><%=((Flight)flight).getPrice() %> </td>
				<td><%=((Flight)flight).getAvailableSeats() %> </td>
				<td>
				<% if(session.getAttribute("userid")!=null ) 
	  {
	  %> 
					 <span>
						<button class="btn btn-dark" onClick="openBooking('<%=((Flight)flight).getFlightId() %>','<%=i%>')">Book</button>
					 </span>
	 	 <% i=-1;
	 	 } %>
				</div>
 	...
 	%>	
 	 ...
 	 <form style="display: none" id="booking" action="TicketBooking" method="post">
  	 <span>
	  	  ...
	  	  <button class="btn btn-dark" type="button">Add more</button>
	 	  <button class="btn btn-dark" type="submit" >Continue Booking</button>
 	  </span>
  	 </form>
 	</div>
